                ...
                child: SingleChildScrollView(
                  ...
                  child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    Container(
                      ...
                      decoration: const BoxDecoration(gradient: LinearGradient(colors: [Color(0xFF6A11CB), Color(0xFF2575FC)]), borderRadius: BorderRadius.all(Radius.circular(16))),
                      child: Row(children: [
                        const Icon(Icons.supervisor_account_rounded, color: Colors.white, size: 32),
                        const SizedBox(width: 12),
                        Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                          const Text('Team Lead', style: TextStyle(color: Colors.white, fontSize: 14, fontWeight: FontWeight.bold)),
                          Text(widget.email, style: const TextStyle(color: Colors.white70, fontSize: 11)),
                        ]),
                      ]),
                    ),
                    const SizedBox(height: 20),
                    Text('TL Overview', style: TextStyle(color: textPrimary, fontSize: 18, fontWeight: FontWeight.bold)),
                    const SizedBox(height: 14),
                    GridView.count(
                      shrinkWrap: true, physics: const NeverScrollableScrollPhysics(),
                      crossAxisCount: 2, crossAxisSpacing: 12, mainAxisSpacing: 12, childAspectRatio: 1.4,
                      children: [
                        _card('Team', '0', Icons.people_rounded, color, cardBg, cardBorder, textPrimary, textSecondary),
                        _card('Sprint', '0', Icons.bolt_rounded, const Color(0xFF4FACFE), cardBg, cardBorder, textPrimary, textSecondary),
                        _card('Done', '0', Icons.check_circle_rounded, const Color(0xFF43E97B), cardBg, cardBorder, textPrimary, textSecondary),
                        _card('Bugs', '0', Icons.bug_report_rounded, const Color(0xFFFA709A), cardBg, cardBorder, textPrimary, textSecondary),
                      ],
                    ),
                  ]),
                ),
      ...
      child: SafeArea(child: Column(children: [
        Container(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            const Text('Team Lead', style: TextStyle(color: Colors.white70, fontSize: 12)),
            Text(widget.userId, style: const TextStyle(color: Colors.white60, fontSize: 11)),
          ]),
        ),
        Expanded(child: ListView(children: [
          ListTile(leading: const Icon(Icons.people_rounded, color: Color(0xFF6A11CB)), title: Text('Team', style: TextStyle(color: textPrimary)), onTap: () => Navigator.pop(context)),
          ListTile(leading: const Icon(Icons.task_rounded, color: Color(0xFF6A11CB)), title: Text('Tasks', style: TextStyle(color: textPrimary)), onTap: () => Navigator.pop(context)),
          ListTile(leading: const Icon(Icons.bug_report_rounded, color: Color(0xFF6A11CB)), title: Text('Bugs', style: TextStyle(color: textPrimary)), onTap: () => Navigator.pop(context)),
        ])),
        const Divider(),
