...
class MarketingTeamDashboard extends StatefulWidget {
  final String email;
  final String firstName;
  final String userId;
  const MarketingTeamDashboard({super.key, required this.email, required this.firstName, required this.userId});

  @override
  State<MarketingTeamDashboard> createState() => _MarketingTeamDashboardState();
}

class _MarketingTeamDashboardState extends State<MarketingTeamDashboard> {
                ...
                child: SingleChildScrollView(
                  ...
                  child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    Container(
                      ...
                      decoration: const BoxDecoration(gradient: LinearGradient(colors: [Color(0xFFFA709A), Color(0xFFFFE985)]), borderRadius: BorderRadius.all(Radius.circular(16))),
                      child: Row(children: [
                        const Icon(Icons.campaign_rounded, color: Colors.white, size: 32),
                        const SizedBox(width: 12),
                        Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                          const Text('Marketing Team', style: TextStyle(color: Colors.white, fontSize: 14, fontWeight: FontWeight.bold)),
                          Text(widget.email, style: const TextStyle(color: Colors.white70, fontSize: 11)),
                        ]),
                      ]),
                    ),
                    const SizedBox(height: 20),
                    Text('Marketing Overview', style: TextStyle(color: textPrimary, fontSize: 18, fontWeight: FontWeight.bold)),
                    const SizedBox(height: 14),
                    GridView.count(
                      shrinkWrap: true, physics: const NeverScrollableScrollPhysics(),
                      crossAxisCount: 2, crossAxisSpacing: 12, mainAxisSpacing: 12, childAspectRatio: 1.4,
                      children: [
                        _card('Campaigns', '0', Icons.campaign_rounded, color, cardBg, cardBorder, textPrimary, textSecondary),
                        _card('Leads', '0', Icons.people_alt_rounded, const Color(0xFF4FACFE), cardBg, cardBorder, textPrimary, textSecondary),
                        _card('Conversions', '0', Icons.trending_up_rounded, const Color(0xFF43E97B), cardBg, cardBorder, textPrimary, textSecondary),
    ...
    return Drawer(
      ...
      child: SafeArea(child: Column(children: [
        Container(
          ...
          decoration: const BoxDecoration(gradient: LinearGradient(colors: [Color(0xFFFA709A), Color(0xFFFFE985)])),
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            const Text('Marketing', style: TextStyle(color: Colors.white70, fontSize: 12)),
            Text(widget.userId, style: const TextStyle(color: Colors.white60, fontSize: 11)),
          ]),
        ),
        Expanded(child: ListView(children: [
          ListTile(leading: const Icon(Icons.campaign_rounded, color: Color(0xFFFA709A)), title: Text('Campaigns', style: TextStyle(color: textPrimary)), onTap: () => Navigator.pop(context)),
          ListTile(leading: const Icon(Icons.people_alt_rounded, color: Color(0xFFFA709A)), title: Text('Leads', style: TextStyle(color: textPrimary)), onTap: () => Navigator.pop(context)),
        ])),
        const Divider(),
